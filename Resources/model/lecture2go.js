...
var Model = function() {
	...
	this.videoDB = Ti.Database.open(DBNAME);
	this.getVideosFromSQL = function(sql) {
		...
		while (_result.isValidRow()) {
			...
			videos.push({
				...
				//	cached : this.getVideoStatus({
				//		filename : _result.fieldByName('filename')
				//	}),
				stream : 'http://fms.rrz.uni-hamburg.de/vod/_definst_/mp4:8l2gbal1768/' + idstr + '/playlist.m3u8',
				image : 'http://lecture2go.uni-hamburg.de/images/' + idstr + '.jpg',
				mp4 : 'http://fms1.rrz.uni-hamburg.de/abo/' + idstr + '.mp4'
			});
			_result.next();
		}
		_result.close();
...
Model.prototype.getVideoList = function(_argc) {
	...
	switch (Ti.App.Properties.getString('menuemodus')) {
		case 'latest' :
			var q = 'SELECT v.filesize,v.dimension, v.duration, v.filename,v.title title,v.hits,v.author,v.publisher,v.id id,c.nr nr,c.lang lang, c.id AS channelid, c.name AS channelname from videos AS v ,channels AS c WHERE c.id=v.channelid AND v.openaccess=1 ORDER BY cdate DESC LIMIT ' + offset + ',' + limit;
			break;
		case 'popular':
			var q = 'SELECT v.filesize,v.dimension, v.duration, v.filename,v.title title,v.hits,v.author,v.publisher,v.id id,c.nr nr,c.lang lang, c.id AS channelid, c.name AS channelname from videos AS v ,channels AS c WHERE c.id=v.channelid AND v.openaccess=1 ORDER BY hits DESC LIMIT ' + offset + ',' + limit;
			break;
		default:
			return;
	}
...
Model.prototype.search = function(_args) {
	...
	var q = 'SELECT v.dimension, v.filename, v.filesize,v.duration,v.title title,v.hits,v.author,v.publisher,v.id id,c.nr nr,c.lang lang, c.id channelid, c.name channelname FROM videos v, channels c WHERE c.id=v.channelid AND v.openaccess=1 AND (title LIKE "%' + needle + '%" ' + 'OR author LIKE "%' + needle + '%" ' + 'OR publisher LIKE "%' + needle + '%" ' + ') ORDER BY cdate DESC LIMIT 0,' + limit;
	_args.onsuccess(this.getVideosFromSQL(q));
}
Model.prototype.getVideoById = function(_id) {
	var q = 'SELECT v.dimension, v.filename, v.filesize,v.duration,v.title title,v.hits,v.author,v.publisher,v.id id,c.nr nr,c.lang lang, c.id channelid, c.name channelname FROM videos v, channels c WHERE c.id=v.channelid AND v.openaccess=1 AND v.id = "' + _id + '"';
	console.log(q);
